...
struct task
	{
		...
		task(juk_msg::juk_set_target_data_msg target_msg_ ,
		juk_msg::juk_dji_camera_control_msg cam_msg_ ,
...
ros::NodeHandle nh;
	...
	void gps_a3_cb(const juk_msg::juk_dji_gps_msg::ConstPtr& m)
	{
	...
	void pos_data_cb(const juk_msg::juk_position_data_msg::ConstPtr& pos)
	{
		pos_data = *pos;
		...
		else
		{
			...
			{
				current_point.cam_msg.action = juk_msg::juk_dji_camera_control_msg::take_photo;
...
//#define TEST_LIDAR
	...
	{		
		...
		if (argc < 2)
		{
			cout << red("Too few arguments") << endl;
			//return -1;
			exit(-1);
		}
	
		ifstream route_file(argv[1]);
		...
		while (std::getline(route_file, line))
		{
			...
			juk_msg::juk_set_target_data_msg msg_target;
			...
			juk_msg::juk_dji_camera_control_msg msg_cam;
...
//			cout << endl;
		...
		target_pub = nh.advertise<juk_msg::juk_set_target_data_msg>("JUK/TARGET", 1);
		camera_pub = nh.advertise<juk_msg::juk_dji_camera_control_msg>("JUK/DJI_GIMBAL", 1);
		action_pub = nh.advertise<juk_msg::juk_navigation_actions_msg>("JUK/NAVIGATION_ACTIONS", 1);
